...
import iss2021_resumablebw.annotations.*;
import iss2021_resumablebw.interaction.*;
...
public class IssArilRobotSupport implements IssCommSupport {
    ...
    protected String translate(String arilMove) {
        ...
        switch (arilMove.trim()) { //translate into critl move
            case "h":
                return "{\"robotmove\":\"alarm\", \"time\": " + timemap.get("h") + "}";
            case "w":
                return "{\"robotmove\":\"moveForward\", \"time\": " + timemap.get("w") + "}";
            case "s":
                return "{\"robotmove\":\"moveBackward\", \"time\": " + timemap.get("s") + "}";
            case "l":
                return "{\"robotmove\":\"turnLeft\", \"time\": " + timemap.get("l") + "}";
            case "r":
                return "{\"robotmove\":\"turnRight\", \"time\":" + timemap.get("r") + "}";
            default:
                return "{\"robotmove\":\"alarm\", \"time\": " + timemap.get("h") + "}"; //to avoid exceptions
        }
    }
